...
- Compressed air (exterior and interior cleaning)
...
The Dell OptiPlex 3080 SFF was chosen as the Proxmox host for the following reasons:
...
- **10th generation Intel CPU** - the i5-10500 supports Intel VT-x and VT-d, both required for virtualization
- **PCIe expansion slot** - the SFF form factor includes a PCIe X1 slot, allowing an additional dual-port NIC to be added for dedicated network segmentation

---

## Why I Used a Refurbished Machine

For this lab, I used a refurbished enterprise desktop to keep the project realistic and cost-effective. Many small businesses reuse older business-class systems or purchase refurbished equipment instead of buying new hardware for every need.

This also made the hardware setup part of the learning process. Before installing Proxmox, I inspected the system, reviewed the internal components, and confirmed that the machine was suitable for virtualization. This connects directly to desktop support and IT asset management work.
...
Before any hardware work began, the target network design was mapped out to confirm the hardware requirements. This diagram shows the planned logical network and the physical segmentation this design requires is what drove the decision to add a second NIC.

![PolkTech homelab network diagram](../../screenshots/phase-0-pre-build/network_diagram.png)
*Planned PolkTech lab network — OPNsense VM separating home LAN from lab VLANs*
...
*CPU cooler reinstalled after thermal paste application — PCIe X1 slot visible and empty in lower portion of frame*
...
The core problem is that the home network router runs its own DHCP server. Without network segmentation, lab VMs would connect directly to the home network and receive IP addresses from the home router, mixing lab traffic with personal home network traffic and making it impossible to control the lab environment properly.
...
- **Onboard Realtek** - Proxmox management interface, connected to home network
- **Intel 82576 port A** - dedicated OPNsense WAN uplink, passes home network traffic only to OPNsense
- **Intel 82576 port B** - spare, reserved for future use

All lab VMs connect to internal virtual bridges inside Proxmox and route through OPNsense. This means the home router never sees lab VM traffic and never assigns IP addresses to lab VMs. The lab network is completely isolated from the home network.
...
*Rear I/O panel onboard Realtek NIC (bottom) and Intel 82576 dual-port PCIe NIC (top, labeled ACT/LNK A and ACT/LNK B)*
...
- Adding a secondary NIC is necessary when running a virtual firewall inside a hypervisor -without it there is no way to properly separate management traffic from VM traffic and prevent the home router from assigning IPs to lab VMs
...
Understanding why network segmentation requires dedicated physical NICs and being able to plan and implement that at the hardware level before any software is installed, is a skill that applies directly to virtualization administrator and junior systems administrator roles.

---

## Future Improvements

- Add a second NVMe drive for additional VM storage if the 256GB becomes a limiting factor
